...
import { generateSyllabus, streamLessonContent, getCourses, deleteCourse as deleteCourseApi, updateQuizScore } from './services/api';
import { 
  Sparkles, 
  BookOpen, 
  CheckCircle, 
...
const App: React.FC = () => {
  ...
  const handleStartModule = async (module: Module) => {
    ...
    try {
      ...
      const stream = streamLessonContent(activeCourse.id, module.id, activeCourse.title, module.title, module.description);
  ...
  const handleQuizComplete = (moduleId: string, scorePercent: number) => {
      ...
      updateQuizScore(moduleId, scorePercent, isCompleted).catch(console.error);
      setCourses(prev => prev.map(c => {
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
                <div className="relative w-full max-w-5xl">
                    ...
                    <button
                        ...
                        className="absolute top-4 right-4 z-50 bg-white dark:bg-slate-800 text-slate-900 dark:text-white w-10 h-10 rounded-full shadow-lg hover:scale-110 transition-all"
                    >
                        ✕
                    </button>
